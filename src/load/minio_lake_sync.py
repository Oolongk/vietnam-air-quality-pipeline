...
def build_sync_plan(
    ...
    resolved_root = local_lake_root.resolve()

    if not resolved_root.exists():
        raise MinioLakeSyncError(f"Local Data Lake chưa tồn tại: {resolved_root}")

    if not resolved_root.is_dir():
        raise MinioLakeSyncError(
            f"Local Data Lake root không phải thư mục: {resolved_root}"
        )

    sync_entries: list[MinioSyncEntry] = []
    ...
    for (
        ...
        zone_root = resolved_root / local_zone
        ...
        for file_path in sorted(zone_root.rglob("*")):
            ...
            relative_path = file_path.relative_to(zone_root).as_posix()
...
def _write_json_atomically(
    ...
    try:
        ...
        raise MinioLakeSyncError(
            f"Không thể ghi MinIO sync summary: {output_path}"
        ) from error
...
def sync_local_lake_to_minio(
    ...
    for entry in sync_plan:
        try:
            upload_file(
                ...
                file_path=(entry.local_path),
                content_type=(_guess_content_type(entry.local_path)),
                settings=resolved_settings,
                client=resolved_client,
            )
        ...
        uploaded_files += 1
        uploaded_bytes += entry.size_bytes

        bucket_counts[entry.bucket_name] += 1
    ...
    local_time = finished_at.astimezone(ZoneInfo("Asia/Ho_Chi_Minh"))
    ...
    summary_path = summary_directory / "minio_sync_summary.json"
    ...
    summary = {
        "pipeline_name": ("local_data_lake_to_minio_sync"),
        "status": status,
        "run_id": run_id,
        "started_at": (started_at.isoformat()),
        "finished_at": (finished_at.isoformat()),
        "duration_seconds": (finished_at - started_at).total_seconds(),
        "planned_files": len(sync_plan),
    ...
    try:
        ...
        raise MinioLakeSyncError(
            f"Dữ liệu đã được đồng bộ nhưng không thể upload sync summary: {error}"
        ) from error
