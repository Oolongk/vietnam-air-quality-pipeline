...
class S3SnapshotConfigurationError(
    ValueError
):
    """Cấu hình S3 Snapshot Uploader không hợp lệ."""


class S3SnapshotValidationError(
    RuntimeError
):
    """Bộ snapshot local không đúng contract."""


class S3SnapshotUploadError(
    RuntimeError
):
...
class S3SnapshotUploadSettings:
    ...
    def from_environment(
        ...
        if not pointer_key.endswith(
            ".json"
        ):
            raise S3SnapshotConfigurationError(
                "AWS_SNAPSHOT_POINTER_KEY "
                "phải kết thúc bằng .json."
            )
...
def _validate_bucket_name(
    ...
    if not BUCKET_NAME_PATTERN.fullmatch(
        ...
        raise S3SnapshotConfigurationError(
            "AWS_SNAPSHOT_BUCKET không phải "
            "tên S3 bucket hợp lệ."
        )
    ...
    if (
        ...
        raise S3SnapshotConfigurationError(
            "AWS_SNAPSHOT_BUCKET chứa "
            "chuỗi dấu không hợp lệ."
        )
...
def build_s3_client(
    ...
    try:
        ...
        raise S3SnapshotConfigurationError(
            "Không tìm thấy AWS profile: "
            f"{settings.profile_name!r}."
        ) from error
        ...
        raise S3SnapshotConfigurationError(
            "Không thể khởi tạo AWS session: "
            f"{error}"
        ) from error
...
class S3SnapshotUploader:
    ...
    def upload(
        ...
        manifest, snapshot_files = (
            self._load_and_validate_snapshot()
        )
        ...
        release_prefix = (
            f"{self.settings.release_root_prefix}"
            f"/{snapshot_id}"
        )
        ...
        try:
            ...
            raise S3SnapshotUploadError(
                "Không thể hoàn tất S3 snapshot "
                f"upload: {error}"
            ) from error
    ...
    def _load_and_validate_snapshot(
        ...
        if not input_directory.exists():
            raise S3SnapshotValidationError(
                ...
                f"directory: {input_directory}"
            )

        if not input_directory.is_dir():
            raise S3SnapshotValidationError(
                "Snapshot input path không "
                f"phải directory: {input_directory}"
            )
        ...
        if not SAFE_SNAPSHOT_ID_PATTERN.fullmatch(
            ...
            raise S3SnapshotValidationError(
                "snapshot_id không an toàn "
                "để dùng trong S3 key."
            )
        ...
        for index, raw_path in enumerate(
            ...
            try:
                resolved_file_path.relative_to(
                    input_directory.resolve()
                )

            except ValueError as error:
                raise S3SnapshotValidationError(
                    "File nằm ngoài snapshot "
                    f"directory: {relative_path}"
                ) from error

            if not resolved_file_path.is_file():
                raise S3SnapshotValidationError(
                    "File được khai báo trong "
        ...
        if "manifest.json" not in (
            seen_files
        ):
            raise S3SnapshotValidationError(
                "Field files phải chứa "
                "manifest.json."
            )
