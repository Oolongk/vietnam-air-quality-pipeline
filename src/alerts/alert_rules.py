...
def _normalize_aqi_value(
    ...
    try:
        ...
        raise AQIClassificationError(f"AQI phải là số, nhận được: {value!r}") from error

    if not numeric_value.is_integer():
        raise AQIClassificationError(
            f"AQI phải là số nguyên, nhận được: {numeric_value}"
        )

    aqi_value = int(numeric_value)

    if aqi_value < 0:
        raise AQIClassificationError("AQI không được âm.")
...
def classify_us_aqi(
    ...
    aqi_value = _normalize_aqi_value(value)
