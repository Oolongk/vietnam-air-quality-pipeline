...
def upload_alert_directory_to_minio(
    ...
    if not alert_directory.exists():
        raise RuntimeError(f"Không tìm thấy thư mục alert được tạo: {alert_directory}")
    ...
    if not files:
        raise RuntimeError("Alert processor không tạo ra file output nào.")
    ...
    for file_path in files:
        ...
        try:
            ...
            raise RuntimeError(f"Không thể đọc file alert tạm: {file_path}") from error

        if not payload:
            raise RuntimeError(f"Không upload file alert rỗng: {file_path}")
...
def main() -> None:
    try:
        ...
        batch_id = str(quality_summary["batch_id"]).strip()

        partition_date = str(quality_summary["partition_date"]).strip()

        partition_hour = str(quality_summary["partition_hour"]).strip()

        clean_object_name = str(quality_summary["clean_object_name"]).strip()
        ...
        if not all(
            ...
            raise RuntimeError("Quality summary thiếu batch hoặc object metadata.")

        clean_dataframe = get_parquet_object(
            bucket_name=(minio_settings.clean_bucket),
        ...
        if clean_dataframe.empty:
            raise RuntimeError("Clean Parquet mới nhất không có dữ liệu.")

        print("Đã chọn batch mới nhất từ MinIO:")

        print(f"Batch ID: {batch_id}")

        print(f"Quality summary: {quality_summary_object_name}")

        print(f"Clean object: {clean_object_name}")

        print(f"Clean rows: {len(clean_dataframe)}")
        ...
        with TemporaryDirectory(prefix="air_quality_alerts_") as temporary_directory:
            ...
            uploaded_objects = upload_alert_directory_to_minio(
                ...
                bucket_name=(minio_settings.mart_bucket),
        ...
        print(f"Batch ID đã xử lý: {batch_id}")

        print(f"Status: {status}")

        print(f"Input records: {input_records}")

        print(f"Số file đã upload lên MinIO: {len(uploaded_objects)}")

        print(f"MinIO bucket: {minio_settings.mart_bucket}")

        print(f"MinIO prefix: {minio_alert_prefix}")

        print()
        print("Các object đã upload:")

        for uploaded_object in uploaded_objects:
            object_name = uploaded_object.get(
                "object_name",
                "UNKNOWN",
            )

            size_bytes = uploaded_object.get(
                "size_bytes",
                0,
            )

            print(f"- {object_name} ({size_bytes} bytes)")
