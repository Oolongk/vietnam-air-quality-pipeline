...
def build_alert_record() -> dict[str, Any]:
    return {
        ...
        "point_id": "HN_CENTER",
        "location_id": "HN",
        "alert_time": ("2026-07-22T08:00:00+07:00"),
        "aqi_value": 175,
        "aqi_level": "Không tốt",
        "severity": "HIGH",
        "message": (
...
def test_latest_alerts_returns_records(
    ...
    assert "from fact_air_quality_alerts" in normalized_query
...
def test_latest_alerts_uses_default_limit(
    ...
    assert captured["parameters"] == (100,)
...
def test_latest_alerts_returns_500_on_database_error(
    ...
    assert "alert" in payload["detail"].lower()
