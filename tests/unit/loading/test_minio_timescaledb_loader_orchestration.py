        ...
        [
            {
                "point_id": "HN_CENTER",
                "location_id": "HN",
                "forecast_time": ("2026-07-25T09:00:00+07:00"),
                "latitude": 21.0285,
                "longitude": 105.8542,
                "source": "open_meteo",
                "batch_id": "BATCH_1",
                "ingested_at": ("2026-07-25T02:15:00+00:00"),
            },
            {
                "point_id": "HCM_CENTER",
                "location_id": "HCM",
...
def test_load_latest_clean_batch_commits_and_writes_summary(
    ...
    minio_settings = build_minio_settings()

    minio_client = object()

    connection = FakeConnection()

    database_settings = FakeDatabaseSettings(connection)

    clean_dataframe = build_clean_dataframe()
    ...
    summary = loader.load_latest_minio_clean_batch(
    ...
    assert ensure_calls[0]["settings"] is minio_settings

    assert ensure_calls[0]["client"] is minio_client

    assert len(parquet_calls) == 1

    assert parquet_calls[0]["bucket_name"] == "air-quality-clean"

    assert parquet_calls[0]["object_name"] == CLEAN_OBJECT

    assert len(upsert_calls) == 1

    assert upsert_calls[0]["connection"] is connection

    assert upsert_calls[0]["expected_batch_id"] == "BATCH_1"

    assert len(upsert_calls[0]["dataframe"]) == 2

    assert (database_settings.connect_count) == 1
    ...
    assert summary["partition_date"] == "2026-07-25"

    assert summary["partition_hour"] == "02"
    ...
    assert summary["database_time_column"] == "forecast_time"

    assert summary["quality_summary_object_name"] == QUALITY_SUMMARY_OBJECT

    assert summary["clean_object_name"] == CLEAN_OBJECT
    ...
    assert summary["summary_object_name"] == expected_summary_object

    assert len(json_writes) == 1

    assert json_writes[0]["bucket_name"] == "air-quality-mart"

    assert json_writes[0]["object_name"] == expected_summary_object

    assert json_writes[0]["data"] == summary
...
def test_load_latest_clean_batch_rolls_back_and_closes_on_upsert_error(
    ...
    monkeypatch.setattr(
        loader,
        "get_parquet_object",
        lambda **kwargs: build_clean_dataframe(),
    )

    def failing_upsert(
        **kwargs: Any,
    ) -> dict[str, Any]:
        raise loader.MinioTimescaleDBLoadError("Upsert test failure")
    ...
    monkeypatch.setattr(
        loader,
        "put_json_object",
        lambda **kwargs: json_writes.append(kwargs),
    )
    ...
    with pytest.raises(
        ...
        (
            loader.load_latest_minio_clean_batch(
                minio_settings=(minio_settings),
                minio_client=(minio_client),
                database_settings=(database_settings),
            )
...
def test_load_latest_clean_batch_rejects_missing_clean_object(
    ...
    invalid_quality_summary = build_quality_summary()

    invalid_quality_summary["clean_object_name"] = "   "
    ...
    assert (database_settings.connect_count) == 0
