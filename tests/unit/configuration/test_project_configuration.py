...
def test_project_has_102_active_monitoring_points() -> None:
    monitoring_points = (
        _read_monitoring_points()
    )

    active_points = _active_rows(
        monitoring_points
    )
    ...
    assert not monitoring_points[
        "point_id"
    ].duplicated().any()


def test_each_location_has_exactly_three_points() -> None:
    locations = _active_rows(
        _read_locations()
    )

    monitoring_points = _active_rows(
        _read_monitoring_points()
    )
...
def test_monitoring_points_reference_existing_locations() -> None:
    ...
    assert point_location_ids.issubset(
        active_location_ids
    )
