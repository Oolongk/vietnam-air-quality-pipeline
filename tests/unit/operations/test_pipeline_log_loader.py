...
def build_test_summaries() -> dict:
    common = {
        ...
        "partition_hour": "11",
        "started_at": ("2026-07-13T04:00:00+00:00"),
        "finished_at": ("2026-07-13T04:00:05+00:00"),
            ...
            "checks": [
                {
                    ...
                    "status": "PASSED",
                    "bad_records_count": 0,
                    "message": ("point_id không được rỗng."),
                },
                {
                    "check_name": ("PM2_5_NON_NEGATIVE"),
                    "status": "PASSED",
                    "bad_records_count": 0,
                    "message": ("pm2_5 không được âm."),
                },
            ],
        },
        "database_load": {
...
def test_builds_five_pipeline_log_records() -> None:
    ...
    records = build_pipeline_log_records(summaries)
...
def test_maps_stage_record_counts() -> None:
    records = build_pipeline_log_records(build_test_summaries())

    records_by_stage = {record["run_id"].split(":")[-1]: record for record in records}

    assert records_by_stage["extraction"]["records_extracted"] == 240

    assert records_by_stage["extraction"]["records_loaded"] == 0

    assert records_by_stage["data_quality"]["records_loaded"] == 240

    assert records_by_stage["database_load"]["records_loaded"] == 240

    assert records_by_stage["alert_processing"]["records_loaded"] == 12


def test_builds_data_quality_logs() -> None:
    records = build_data_quality_log_records(build_test_summaries())

    assert len(records) == 2

    assert all(record["run_id"] == f"{BATCH_ID}:data_quality" for record in records)
...
def test_rejects_mixed_batch_ids() -> None:
    ...
    summaries["transform"]["batch_id"] = "another_batch"

    with pytest.raises(
        PipelineLogLoadError,
        match="không cùng batch_id",
    ):
        build_pipeline_log_records(summaries)


def test_rejects_timestamp_without_timezone() -> None:
    summaries = build_test_summaries()

    summaries["extraction"]["started_at"] = "2026-07-13T04:00:00"
