...
def test_process_batch_writes_clean_bad_and_quality_outputs(
    ...
    def fake_run_data_quality(
        **kwargs: Any,
        ...
        captured_quality_arguments.update(
            kwargs
        )
    ...
    assert len(
        ensure_buckets_calls
    ) == 1

    assert ensure_buckets_calls[0][
        "settings"
    ] is settings
    ...
    assert captured_quality_arguments[
        "expected_batch_id"
    ] == BATCH_ID
