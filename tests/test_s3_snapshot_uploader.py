...
SNAPSHOT_ID = (
    "20260723T090000Z_s3test01"
)

BUCKET_NAME = (
    "vietnam-air-quality-snapshots-test"
)
...
def test_settings_read_sydney_region(
    ...
    assert settings.bucket_name == (
        BUCKET_NAME
    )

    assert settings.release_root_prefix == (
        "releases"
    )

    assert settings.pointer_key == (
        "current.json"
    )